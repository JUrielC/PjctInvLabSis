...
const post_baja = async (req, res) => {
    await pool.getConnection().then(async (conn) => {
        ...
        try {
            ...
            if (parseInt(validation_herramienta_activa[0].id_estatus) === 2) {
                const result = return_error(400, 'No realizado, herramienta tiene un préstamo en curso');
                conn.release();
                return res.status(400).json(result)
            }
...
//no put
const delete_clear_bajas = async (req, res) => {
    await pool.getConnection().then(async (conn) => {

        try {

            //query
            await conn.query("CALL  clear_bajas()");
            res.status(200).json({
                "ok": true,
                "message": {
                    "code": 200,
                    "messageText": "Registros eliminados con éxito"
                }
            })
            conn.release();
...
const delete_baja = async (req, res) => {
    await pool.getConnection().then(async (conn) => {
        ...
        try {
            ...
            const id_baja = req.params.id_baja

            //query
            await conn.query("CALL  delete_baja(?)", id_baja);
            res.status(200).json({
                "ok": true,
                "message": {
                    "code": 200,
                    "messageText": "Registro eliminado con éxito"
                }
            })
            conn.release();
